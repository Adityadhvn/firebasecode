...
export class MemStorage implements IStorage {
  ...
  private seedData() {
    ...
    const performer3: Performer = {
      id: this.performerIdCounter++,
      name: "Techno Pioneers",
      imageUrl: "https://images.unsplash.com/photo-1571266692345-8c7c0c6c8084",
      eventId: event2.id,
      time: "23:00 - 01:00",
      isHeadliner: true,
    };
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByUsername(username: string): Promise<User | undefined> {
    try {
        ...
        throw error;  // Let the server handle the error properly
    }
}
  ...
  async updateTicketType(id: number, ticketTypeUpdate: Partial<InsertTicketType>): Promise<TicketType | undefined> {
    ...
    type DBCompatibleTicketType = Omit<InsertTicketType, "price"> & {
      price?: string;
    };
  
    const updateData: Partial<DBCompatibleTicketType> = {
      ...ticketTypeUpdate,
      price:
        typeof ticketTypeUpdate.price === "number"
          ? ticketTypeUpdate.price.toFixed(2)
          : ticketTypeUpdate.price,
    };
  ...
  async getEventsByOrganizer(organizerId: number): Promise<Event[]> {
    return db
      .select()
      .from(events)
      .where(eq(events.organizedById, organizerId))
      .orderBy(desc(events.date));
  }
}
